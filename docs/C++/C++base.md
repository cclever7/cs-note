...
归纳：当一个对象被用作右值的时候，用的是对象的值；当对象被用作左值的时候，用的是对象的身份(在内存中的位置）
...
1. 对于**内置类型**，低精度的变量给高精度变量赋值会发生隐式类型转换，例如：`double a = int b;`，特别地使用`+=, -=`也会进行类型转换
...
透彻讲解右值引用、移动语义、完美转发链接：

https://www.jianshu.com/p/d19fc8447eaa

https://www.jianshu.com/p/31cea1b6ee24

右值引用就是必须绑定到右值（**一个临时对象、将要销毁的对象**）的引用，一般表示对象的值。右值引用的符号一般为`&&`

```c++
int &&r = i * 42;
```

`std::move()`方法来将左值转换为右值，从而方便应用移动语义
...
<img src="medium/image-20200815094236854.png" alt="image-20200815094236854" style="zoom: 67%;" />
